...
export async function GET() {
  ...
  return NextResponse.json({
    slots:   meta.collections ?? [null, null, null],
    btnLink: meta.collectionsBtnLink ?? '#',
  });
}
...
export async function POST(req: NextRequest) {
  ...
  const btnLink = form.get('btnLink') as string | null;
  ...
  const meta = await readMeta();
  const slots: (string | null)[] = meta.collections ?? [null, null, null];
  slots[index] = filename;
  meta.collections = slots;
  if (btnLink) meta.collectionsBtnLink = btnLink;
